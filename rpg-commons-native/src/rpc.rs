...
impl Lights for Rpc {
    async fn brightness(&self, level: i32) -> Result<(), LightsError> {
        ...
        if let Err(status) = self
            ...
            .await
            .brightness(call)
            .instrument(info_span!(target: "lights:rpc", "brightness", "Setting brightness to {}", level))
            .await
        {
...
impl lights_server::Lights for AppContext {
    async fn brightness(&self, request: Request<BrightnessRequest>) -> Result<Response<BrightnessResponse>, Status> {
        ...
        match self
            ...
            .instrument(
                info_span!(target: "rpc:lights", "brightness", "Handling brightness request to {}", request.level),
            )
            .await
        {
            Ok(()) => Ok(Response::new(BrightnessResponse {})),
            Err(error) => Err(Status::internal(format!("Brightness setting error: {:?}", error))),
        }
    }
}
